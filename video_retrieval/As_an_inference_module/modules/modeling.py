...
class CLIP2Video(CLIP2VideoPreTrainedModel):
    ...
    def _similarity_TDB(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        if self.task_config.EMCL:
            b, n = visual_output.size()
            all_embedding = torch.cat((visual_output, sequence_output), dim=0)
            all_embedding = self.emcl(all_embedding, if_train=False)
            visual_output = all_embedding[:b, :]
            sequence_output = all_embedding[b:, :]
