...
class CENet(BaseModel):
  ...
  def __init__(self,
    ...
    self.em = em
    self.em_set = em_set
    print("normalize_experts", self.normalize_experts)

    if em:
        k = em_set["k"]
        momentum = em_set["momentum"]
        stage_num = em_set["stage_num"]
        lamd = em_set["lamd"]
        beta = em_set["beta"]
        self.emcl = Emcl(k=k, m=len(self.modalities), stage_num=stage_num,
                         momentum=momentum, lamd=lamd, beta=beta)
    ...
    # If Bert architecture is employed for video
      ...
      self.video_GU = nn.ModuleDict()
      for mod in self.modalities:
        self.video_GU[mod] = GatedEmbeddingUnit(same_dim,
                                               same_dim,
                                               use_bn=True,
  ...
  def forward(self,
    ...
    if self.txt_agg[:4] in ['bert']:
      ...
      if self.post_agg == 'cls':
        if self.txt_post == "all":
          txt_experts = collections.OrderedDict()
          for idx, mod in enumerate(self.modalities):
            txt_experts[mod] = last_layer[:, idx]
        text = last_layer[:, 0]
    ...
    if self.vid_cont == 'bert':
      ...
      for _, modality in enumerate(self.modalities):
        ...
        experts[modality] = self.video_GU[modality](experts[modality])
    ...
    if out == "test":
      ...
      text_embds_list = []
      for idx, mod in enumerate(self.modalities):
        vid_embds_list.append(experts[mod].unsqueeze(0))
        text_embds_list.append(text_embd[mod].view(b, -1).unsqueeze(0))
      vid_embds = th.cat(vid_embds_list, 0)
      text_embds = th.cat(text_embds_list, 0)
      all_embds = th.cat((vid_embds, text_embds), 1)
      
      _vid_embds = collections.OrderedDict()
      _text_embds = collections.OrderedDict()

      if self.em:
        all_embds = self.emcl(all_embds, if_train=False)
      m, batch_size, n = vid_embds.size()
      
      for idx, mod in enumerate(self.modalities):
        _vid_embds[mod] = all_embds[idx, :batch_size, :]
        _text_embds[mod] = all_embds[idx, batch_size:, :]

      vid_embds_list = []
      text_embds_list = []
      for idx, mod in enumerate(self.modalities):
        vid_embds_list.append(_vid_embds[mod].unsqueeze(1))
        text_embds_list.append(_text_embds[mod].unsqueeze(1))
    ...
    if out == "embds":
      ...
      if self.em:
        all_embds = self.emcl(all_embds, if_train=True)
      m, batch_size, n = vid_embds.size()
      ...
      cross_view_conf_matrix = sharded_cross_view_inner_product(
        vid_embds=_vid_embds,
        text_embds=_text_embds,
        vid_weights=vid_weights,
        text_weights=text_weights,
...
class ReduceDim2(nn.Module):
  ...
  def __init__(self, input_dimension, output_dimension, k_size=5):
    super(ReduceDim2, self).__init__()
    self.conv = nn.Conv1d(input_dimension, output_dimension, kernel_size=k_size, padding=(k_size-1)//2)
    self.relu = nn.ReLU()
    self.fc = nn.Linear(output_dimension, output_dimension)
...
class EncoderReduceDim(nn.Module):

  def __init__(self, input_dimension, output_dimension=512, k_size=1):
    super(EncoderReduceDim, self).__init__()
    self.conv = nn.Conv1d(input_dimension, 1, kernel_size=k_size, padding=(k_size-1)//2)
    self.relu = nn.ReLU()
    self.fc = nn.Linear(output_dimension, output_dimension)
...
class Emcl(object):
    def __init__(self, k, m, stage_num, momentum, lamd=1, beta=3):
        self.k = k
        self.lamd = lamd
        self.stage_num = stage_num
        self.beta = beta
        self.momentum = momentum
        self.mu = th.Tensor(m, 1, self.k).cuda()
    ...
    def __call__(self, embds, if_train=True):
        m, b, n = embds.size()
        mu = self.mu.repeat(1, b, 1)
        _embds = embds
        with th.no_grad():
            for i in range(self.stage_num):
                _embds_t = _embds.permute(0, 2, 1)             # m * n * b
                z = th.bmm(_embds_t, mu)                    # m * n * k
                z = self.lamd * z
                z = F.softmax(z, dim=2)
                z = z / (1e-6 + z.sum(dim=1, keepdim=True))
                mu = th.bmm(_embds, z)                      # m * b * k
                mu = mu / (1e-6 + mu.norm(dim=1, keepdim=True))
        z_t = z.permute(0, 2, 1)                               # m * k * n
        _embds = th.bmm(mu, z_t)                            # m * b * n

        if if_train:
            self.mu = self.momentum * self.mu + (1 - self.momentum) * mu.mean(dim=1, keepdim=True)

        return nn.functional.normalize(embds + self.beta * _embds, dim=-1)
